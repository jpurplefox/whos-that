...
@pytest.mark.asyncio
async def test_save_assigns_id_to_new_game(pikachu: Pokemon) -> None:
...
@pytest.mark.asyncio
async def test_save_preserves_existing_id(pikachu: Pokemon) -> None:
    repository = InMemoryGameRepository()
...
@pytest.mark.asyncio
async def test_get_returns_saved_game(pikachu: Pokemon) -> None:
    repository = InMemoryGameRepository()
    game = Game(pokemon=pikachu)
    saved_game = await repository.save(game)
...
@pytest.mark.asyncio
async def test_get_returns_copy_not_reference(pikachu: Pokemon) -> None:
    repository = InMemoryGameRepository()
    game = Game(pokemon=pikachu)
    saved_game = await repository.save(game)
...
@pytest.mark.asyncio
async def test_save_stores_copy_not_reference(pikachu: Pokemon) -> None:
    repository = InMemoryGameRepository()
    game = Game(pokemon=pikachu)
    saved_game = await repository.save(game)

    saved_game.max_attempts = 10
