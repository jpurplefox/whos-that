...
async def test_capture_new_pokemon_creates_entry_with_times_caught_1(
    ...
    result = await capture.execute("user-1", pikachu)

    assert result.user_id == "user-1"
    assert result.pokemon.id == 25
...
async def test_capture_existing_pokemon_increments_times_caught(
    ...
    await capture.execute("user-1", pikachu)
    result = await capture.execute("user-1", pikachu)
...
async def test_first_caught_at_does_not_change_on_subsequent_captures(
    ...
    first_capture = await capture.execute("user-1", pikachu)
    second_capture = await capture.execute("user-1", pikachu)
